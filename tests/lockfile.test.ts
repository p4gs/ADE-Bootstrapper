...
const CP = { length: 0, headHash: AUDIT_GENESIS };
...
describe("lockfile", () => {
  test("ISC-35/36/40: records file hashes, environment tools, and ade version", async () => {
    ...
    const lock = await generateLockfile(ctx, PATHS, CP);
  ...
  test("ISC-37: serialization is byte-deterministic and timestamp-free", async () => {
    ...
    const first = serializeLockfile(await generateLockfile(ctx, PATHS, CP));
    const second = serializeLockfile(await generateLockfile(makeTestCtx(dir), [...PATHS].reverse(), CP));
  ...
  test("ISC-109: tool version drift is informational, never a failure", async () => {
    ...
    const lock = await generateLockfile(lockCtx, PATHS, CP);
  ...
  test("loadLockfile: absent and corrupt both return null", async () => {
    ...
    expect(await loadLockfile(dir)).toBeNull();
    const ctx = makeTestCtx(dir);
    await Bun.write(join(dir, LOCKFILE_NAME), serializeLockfile(await generateLockfile(ctx, PATHS, CP)));
