...
- [x] ISC-204: all work committed with Justin's Secretive-signed commit (tap prompt — never auto-signed); tree clean at close
...
- [x] ISC-301: [CLOSED 2026-08-03 — see Verification] a real Tier-3 "what did it catch" metric for Secret Scanning needs the secrets module itself to change — the native hook shim would need to redirect TruffleHog's `--json` output to a retained file (e.g. `.ade/logs/secrets-scan.jsonl`) instead of `/dev/null` before ADE could parse a finding count from it; that is a scope-widening change to `modules::secrets.rs`'s hook contract (a new artifact a hook writes on every commit), not "parse existing output," so it was correctly left undone per this task's own rule rather than faked. Gitleaks has the same gap plus a prior one: it is not invoked by the secrets module at all today.
...
- 2026-08-03T03:45Z — **ISC-190 live proof: HALF-CLOSED honestly.** The release tray ran LIVE on this machine under load 68: alive and responsive after 10s (no crash, no hang — the catch_unwind poll-loop contract holding in a real process) and exited cleanly on TERM. The other half — the menu VISIBLY opening with the degraded notice — has two honest blockers: (a) degraded mode's only trigger is an in-process detection panic, deliberately not injectable from outside a release binary, and (b) menu-open AX/pixel verification hits this machine's DOCUMENTED TCC gap (TF-MENUBAR, PROJECTS.md: the automation process holds no Accessibility/Screen-Recording grant — proven across 5 independent methods on 2026-07-20). The deterministic `menu_plan(None)` tests remain the degraded-content proof. ISC-190 stays OPEN pending either a TCC grant or the owner glancing at the menu bar with detection sabotaged. **Also this stretch: the ISC-301 commit is STAGED with message ready but TAP-BLOCKED — `ssh-keygen -Y sign` failed twice with "communication with agent failed" (Secretive agent unreachable/prompt expired); per the standing rule this blocked state is correct, no retry-looping, no unsigned fallback.**
- 2026-08-03T03:15Z — **Post-authorization stretch: branch pushed + PR #1 CI green; ISC-204 and ISC-301 closed; ISC-281/303 attempt correctly withheld.** Owner authorized push + "keep working on any and all remaining ISCs." Pushed `feat/phase-j-design-system`; ci.yml is main/PR-gated so PR #1 was opened per the watch-CI rule — oracle/parity/rust/Socket all PASS. ISC-301 implemented (see Verification — the parity harness caught a genuinely-unapplied TS edit mid-implementation, exactly the failure class it exists for). ISC-281/ISC-303's live-capture attempts stay deferred on their own sparing rule: `uptime` read load **271** at attempt time — worse than the 100-124 that produced byte-identical broken captures and the 228 that deferred the cycle-1 checkpoint; burning capture budget under a proven-broken condition would be waste, not diligence. Remaining open: ISC-190 (tray degraded-mode live proof — next), ISC-281/303 (need a quiet machine), ISC-310 (standing owner loop).
...
- ISC-204: commit `ef49ff2` (Phase J) signed with the owner's Secretive key — `%GK` = `SHA256:p0ivaYBm/U+l/lZ0QPgni8KVLsEbC6Bg2VFtldUsF74`, `%G?` = G, author Justin Pagano — his physical tap, never auto-signed; tree clean at that close (0 dirty). The ISC-301 increment landed the same way (see its own commit).
- ISC-301: the Tier-3 "what did it catch" metric is REAL end-to-end. Hook contract: the native shim now runs TruffleHog with `--json` into a temp file and appends ONE redacted count line per scan to `.ade/logs/secrets-scan.jsonl` (`{"ts":..,"findings":N,"result":"blocked"|"clean"}`) — deliberately counts-only, because TruffleHog's raw JSON carries the secret VALUES and retaining them would be a secrets-retention bug (deviation from the ISC's literal "redirect --json output to a retained file", improved on security grounds, in-code comment). `.ade/logs/` joins GITIGNORE_LINES on both sides. Both templates byte-identical: live `bun` expansion of the TS `hookScript()` diffed against the Rust byte-pin — 1814 bytes, equal (caught TWO real divergences on the way: the TS edit initially not landing at all — exposed by parity-check FAIL on .gitignore, the oracle doing its job — and a TS template-literal `\n` escape emitting a real newline where printf needs backslash-n). The metric block itself executed empirically in real `/bin/sh` (2 findings → `{"findings":2,"result":"blocked"}`, parses). Parsing: `stats::parse_secrets_log` (torn lines skipped, zero-valid → None), `merge_secrets_logs` across projects, wired via `refresh_group_stats` fs-reads into `GroupStats.secrets_catches`, rendered as the "SECRETS CAUGHT" tile ("3 commits / 5 verified findings across 41 scans", golden reviewed). HOOK_MARKER stays v1 ON PURPOSE — it is a recognition sentinel (`contains`), and bumping it would make v1 installs read as foreign hooks and get chained; `apply` refreshes content on every run regardless. Gitleaks remains un-invoked by the module (pre-existing gap, out of ISC-301's own scope statement). Gates: 629 tests/0, parity PASS, all seven green.
